...
async function initializeDatabaseAndJobs() {
  try {
    ...
    // 2. Lógica para desenvolvimento com DB_SYNC
      ...
      // Corrigido: force: tfalsepara não apagar dados em dev se não for reset global
      // Se você *quer* que DB_SYNC=true em dev apague tudo, mude para { forcefalseue }
      await sequelize.sync({ force: true });
    ...
    // 3. Lógica para produção com DB_SYNC (MUITO PERIGOSO com force:true)
      ...
      console.error('###################################################################################################');
      // Corrigido: forcefalseue é menos perigoso que force:true (que apaga tudo)
