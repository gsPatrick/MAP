...
1.  **Saudação Criativa e Temática (Para Ações):** QUANDO UMA OU MAIS AÇÕES FOREM DETECTADAS (e todos os dados obrigatórios estiverem presentes), sua primeira frase (no campo \`overall_summary_suggestion\`) DEVE ser uma saudação curta, criativa e temática, relacionada ao conteúdo da(s) ação(ões) do usuário.
  ...
  "clarifications_needed": [ // Preencher SE E SOMENTE SE um dado obrigatório faltar ou houver ambiguidade. Priorize pedir um dado faltante por vez.
    {
      ...
      "segment_text": "string", // A mensagem original do usuário
      "clarification_question": "string" // Pergunta AMIGÁVEL COM EXEMPLO CORRIGIDO.
    }
  ],
  "ununderstood_segments": [ "string" ],
  "reply_to_user_suggestion": "string" // Se clarifications_needed, esta é a clarification_question. Senão, é a resposta normal.
}
...
17. GENERAL_GREETING_OR_SMALLTALK: (Sem parâmetros)
18. ACTION_CONFIRMATION_YES: (Sem parâmetros)
19. ACTION_CONFIRMATION_NO: (Sem parâmetros)
20. GENERAL_QUESTION_OR_HELP: (Sem parâmetros)
...
- \`reply_to_user_suggestion\`:
    ...
    - Se ações detectadas (sem clarificações): frase de transição curta.
...
- Compra de jogo: "🎮✨ Olá, ${clientNameForPrompt}! Diversão garantida à vista! 😄"
- Lembrete de pagar pai: "⏰ Tic-tac, ${clientNameForPrompt}! Agendando esse lembrete pro paizão! 😉"
...
async function interpretUserMessage(userMessage, conversationContext = {}) {
  if (!OPENAI_API_KEY) {
    ...
    return { /* ... fallback de erro ... */ };
  }

  const clientNameForPrompt = conversationContext.clientName || "[Nome do Usuário]";
  const systemPromptContent = buildSystemPrompt(conversationContext); // Nome do cliente já é usado dentro de buildSystemPrompt
  ...
  const messagesToSendToAPI = [
      ...
      ...conversationHistoryForAPI.slice(-4), // Últimas 2 interações
  ...
  try {
    const completion = await openai.chat.completions.create({
      model: process.env.OPENAI_MODEL || "gpt-3.5-turbo-1106", // Recomendo "gpt-4o" ou "gpt-4-turbo-preview" para melhor desempenho
      messages: messagesToSendToAPI,
      temperature: 0.2, // Mais baixo para mais precisão na extração e menos "alucinação"
