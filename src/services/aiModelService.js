...
function buildSystemPrompt(conversationContext) {
  ...
  **DIFERENCIAÇÃO CRUCIAL: TRANSAÇÃO IMEDIATA vs. LIQUIDAR CONTA EXISTENTE vs. LEMBRETE/COMPROMISSO FUTURO vs. RECORRÊNCIA vs. COMPRA PARCELADA NO CARTÃO:**
  ...
  -   **PAGAMENTO DE FATURA DE CARTÃO (NÃO É CONTA PENDENTE):** Quando o usuário diz que pagou/quitou/antecipou a **fatura** de um cartão (ex: "paguei a fatura do Renner", "quitei o cartão Visa", "já paguei fatura do cartão Renner", "antecipei a fatura"), use **SETTLE_OPEN_CREDIT_CARD_INVOICE** com \`creditCardName\` (se não informou valor) ou **PAY_CREDIT_CARD_INVOICE** (se informou valor exato). **NUNCA** use \`MARK_TRANSACTION_AS_PAID_RECEIVED\` para faturas de cartão — a fatura é calculada pelos gastos no cartão, não é uma conta pendente com descrição "Fatura do cartão X". Funciona mesmo antes do vencimento (dia 15 etc.): o sistema liquida a fatura **aberta** do ciclo atual.
  -   **LIQUIDAR CONTA/RECORRÊNCIA EXISTENTE (sem valor novo):** Use \`MARK_TRANSACTION_AS_PAID_RECEIVED\` **SOMENTE** quando o usuário diz "paguei/quitei" uma conta, assinatura ou recorrência **SEM informar um valor novo** (ex: "paguei o aluguel", "paguei a Netflix", "paguei a conta de luz", "quitei o condomínio"). NÃO peça valor nem forma de pagamento (o sistema já tem). **NUNCA** use MARK se a mensagem contiver "gastei/comprei/recebi" + valor, ou "paguei [valor] reais/em/de...", ou se mencionar **fatura de cartão**.
  ...
  **3. DETECÇÃO DE AÇÃO ESPECÍFICA (SE DADOS ESTIVEREM COMPLETOS):**
    - Se as condições 1 e 2 não se aplicam, analise a mensagem do usuário para encontrar a ação mais apropriada, seguindo a lógica abaixo:
      ...
      b2. **Liquidar conta/recorrência existente:**
          ...
          - O usuário diz que pagou/quitou/antecipou a **fatura de cartão** (ex: "paguei fatura do Renner", "quitei o cartão Visa")? Use \`SETTLE_OPEN_CREDIT_CARD_INVOICE\` — **NUNCA** \`MARK_TRANSACTION_AS_PAID_RECEIVED\`.
