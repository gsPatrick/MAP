  ...
  async criarPreferenciaAssinatura(clientId, planId) {
    try {
      ...
      const preferencePayload = {
        ...
        // <<< ADICIONE ESTA LINHA PARA CORRIGIR O ERRO NO APLICATIVO >>>
  ...
  async processarWebhook(dados) {
    ...
    try {
      ...
      if (paymentData.status === "approved" && subscription.status !== 'Ativa') {
        ...
        // Assumindo que você tem um serviço para ativar a assinatura
        await subscriptionService.updateSubscriptionStatusByExternalId(subscription.externalSubscriptionId, 'Ativa', new Date());
