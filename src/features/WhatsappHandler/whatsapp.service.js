...
function formatAppointmentDataStructure(appointment, forReminder = false) {
    ...
    if (forReminder) {
        introEmoji = "🔔 LEMBRETE";
        introText = `Compromisso Próximo (${appointment.client?.name || 'Geral'})`;
    }

    let data = `${introEmoji} ${introText}:\n\n`;
    data += `💼 Título: *${appointment.title || 'N/A'}*\n`;
    data += `📆 Data: ${formatDate(appointment.eventDateTime)}\n`;
    data += `🕔 Horário: ${formatTime(appointment.eventDateTime, false)}\n`;
    ...
    if (appointment.durationMinutes) {
        ...
        data += `🕔 Término Estimado: ${formatTime(endTime, false)}\n`;
    }
...
function formatRecurringRuleDataStructure(rule) {
    ...
    frequencyText = freqMap[rule.frequency] || frequencyText;
...
function formatSharedAccessDataStructure(sharedAccess, perspective = 'owner') {
    ...
    data += `🚦 Status do Convite/Acesso: *${translateStatus(sharedAccess.status)}*\n`; // Status pode ser Active, Pending, Inactive
...
function formatListSharedAccessDataStructure(accessList, perspective = 'owner') {
    ...
    accessList.forEach((sa, index) => {
        ...
        data += ` - Status: *${translateStatus(sa.status)}*\n`;
    });
    return data.trim();
}
...
function formatAccountSelectionMessage(clientName, planDetailsText, accounts, ownerNameIfShared = null) {
    let aiIntro = `👋 Que bom te ver por aqui, ${clientName}! ${ownerNameIfShared ? `Você está gerenciando as contas de *${ownerNameIfShared}* e ` : ''}${planDetailsText !== "Nenhum plano ativo" ? `Seu ${planDetailsText} está a todo vapor!` : 'Vamos colocar suas finanças em dia?' } 🚀`;
...
async function initializeOrUpdateState(client, sharedAccessRecord = null, existingState = null, clientAccountsFromDb = [], ownerAccountsIfShared = []) {
    ...
    let ownerClientNameForContext = clientName; // Por padrão, o nome do dono é o nome do ator.
    ...
    if (sharedAccessRecord) {
        ...
        sharedAccessPermissions = {
            ...
            // Adicione outras permissões que seu sharedAccessService possa ter
            // Ex: canEdit: sharedAccessRecord.permissions?.canEdit || false,
        };
        ...
        if(ownerClientTemp) {
            ...
            ownerClientNameForContext = "Dono(a) da Conta"; // Fallback
        }
    ...
    if (ownerClientForContext.accessLevel && ownerClientForContext.accessLevel !== 'gratuito') {
        if (ownerClientForContext.accessLevel.startsWith('vitalicio_')) {
            ...
            if (expiryDate >= today) {
                ...
                clientAccessLevel = 'gratuito'; // Downgrade local
            }
        } else {
    ...
    if (hasPaidAccess) { // Acesso PAGO (do dono) está ativo
        if (onboardingStage === 'awaiting_plan_confirmation' || (existingState && !existingState.hasPaidAccess_whenStageLastSet) ) {
            if (!client.email || !client.passwordHash) { // Onboarding de credenciais do ATOR
                ...
                if (!isSharedAccessContext) { // Se o ator é o dono
                    ...
                    if (!hasPfActor) {
                         onboardingStage = 'setting_up_pf_account_name'; // Dono cria sua PF
                    } else {
                         ...
                         if (planTier === 'avancado' && !hasPjMeiActor &&
                            ...
                            onboardingStage = 'confirming_pj_mei_setup'; // Dono pode criar seu PJ/MEI
                         } else {
                            onboardingStage = 'onboarding_complete';
                         }
                    }
                } else { // É acesso compartilhado, e ator já tem credenciais.
                    onboardingStage = 'onboarding_complete'; // Vai direto para seleção de conta do dono.
                }
            }
        }
    } else { // Sem acesso pago (do dono)
        onboardingStage = 'awaiting_plan_confirmation';
    }

    // Tenta definir conta default SE onboarding completo e tem acesso pago e há contas operáveis
    ...
    if (onboardingStage === 'onboarding_complete' && hasPaidAccess && accountsForOperation.length > 0) {
        ...
        if (!defaultAccount && accountsForOperation.length === 1) { // Se só uma conta, torna ela a default "de fato"
            defaultAccount = accountsForOperation[0];
        } else if (!defaultAccount) { // Se múltiplas e nenhuma default, pega a primeira PF, depois PJ, depois MEI
            defaultAccount = accountsForOperation.find(a => a.accountType === 'PF') ||
                             ...
                             accountsForOperation[0]; // Último caso, pega a primeira da lista
        }
    }
    ...
    if (existingState) {
        ...
        if (onboardingStage === 'onboarding_complete' && hasPaidAccess) {
            // Se não tem conta ativa, ou a conta ativa não está mais na lista de contas operáveis
            ...
            if (!currentActiveStillValid && defaultAccount) {
                ...
                // Se não há default e a ativa não é válida, mas HÁ contas operáveis, limpa a ativa para forçar seleção.
        ...
        logger.debug(`[WHATSAPP SERVICE - UpdateState] Estado atualizado para ator ${client.id}: `, {
            ...
            hasPaidAccess: existingState.hasPaidAccess, // Do dono
            activeAccountId: existingState.activeFinancialAccountId, // Da conta do dono
            activeAccountName: existingState.activeFinancialAccountName,
            accessLevelTextForUser: existingState.accessLevelTextForUser, // Do dono
    ...
    logger.debug(`[WHATSAPP SERVICE - InitializeState] Novo estado criado para ator ${client.id}: `, {
        ...
        hasPaidAccess: newState.hasPaidAccess, // Do dono
        accessLevelTextForUser: newState.accessLevelTextForUser, // Do dono
...
async function processIncomingMessage(senderPhoneNormalized, messageText, pushName, rawPayload) {
    ...
    let state;
    let actorClient; // Definido no escopo mais amplo
    ...
    try {
        ...
        if (actorClient) {
            ...
            if (sharedAccessRecord) {
                ...
                if (!actorClient || !actorClient.isActive) { // Verifica se o ator convidado existe e está ativo
                ...
                if (ownerAccountsIfShared.length === 0 && (sharedAccessRecord.canAccessPersonalProfile || sharedAccessRecord.canAccessBusinessProfileId)) {
                     ...
                     return;
                } else if (ownerAccountsIfShared.length === 0){
                ...
                // Mensagem inicial de boas-vindas já direciona para o fluxo de plano
        ...
        logger.debug(`[WHATSAPP ONBOARDING ENTRY] Ator: ${actorClient.id}, DonoCtx: ${state.ownerClientIdForContext}, Stage: ${state.data.onboardingStage}, currentAction: ${state.currentAction}, hasPaidAccess (Dono): ${state.hasPaidAccess}, accessLevelText (Dono): ${state.accessLevelTextForUser}, isShared: ${state.isSharedAccessContext}`);

        // ---- FLUXO DE ONBOARDING (DO ATOR) ----
        if (state.data.onboardingStage === 'awaiting_plan_confirmation') {
            // A mensagem de "welcome" já foi enviada se for um usuário totalmente novo.
            // Se chegou aqui por outro motivo (ex: plano expirou), envia.
            if(state.hasPaidAccess_whenStageLastSet || isNewUserForSessionLogic === false) { // Se tinha acesso antes ou não é novo na sessão.
                onboardingReply = getOnboardingWelcomeNoPlanMessage(clientNameToUse); // Nome do ator
            }
            state.currentAction = 'awaiting_plan_interest_generic';
        } else if (state.data.onboardingStage === 'setting_up_credentials_email') {
            // (Lógica de onboarding de credenciais do ator mantida)
            ...
            if (nameInput.length >= 3 && nameInput.includes(" ")) {
                try {
                    ...
                    if (updatedActorClient) actorClient = updatedActorClient; // Atualiza a variável actorClient
            ...
            // (Lógica de onboarding de conta PF do ator mantida)
             if (state.isSharedAccessContext) { 
                ...
                onboardingReply = getOnboardingAskForPFAccountNameMessage(clientNameToUse);
                state.currentAction = 'awaiting_input_pf_name';
            } else { /* ... processa nome da conta PF ... */ 
            ...
            // (Lógica de onboarding de conta PJ/MEI do ator mantida)
            if (state.isSharedAccessContext) {
                state.data.onboardingStage = 'onboarding_complete'; state.currentAction = null;
                ...
                state.currentAction = 'awaiting_pj_mei_confirm';
            } else { /* ... processa confirmação PJ/MEI ... */ 
            ...
            // (Lógica de onboarding de tipo PJ/MEI do ator mantida)
            ...
            // (Lógica de onboarding de nome PJ/MEI do ator mantida)
            if (state.isSharedAccessContext) { state.data.onboardingStage = 'onboarding_complete'; state.currentAction = null; }
            else {
        ...
        if (state.data.onboardingStage === 'onboarding_complete' || !onboardingReply) {
            ...
            if (rawPayload && rawPayload.selectedButtonId && typeof rawPayload.selectedButtonId === 'string') {
                ...
                // Validação de permissão para edição/exclusão em contexto compartilhado
                const canEditOrDelete = !state.isSharedAccessContext; // Simplificado: só dono pode. Ajustar com state.sharedAccessPermissions.canEditXYZ se houver.
                ...
                if (buttonId.startsWith('edit_')) {
                    if (!canEditOrDelete) {
                     ...
                     if (!canEditOrDelete) {
                        ...
                        if (buttonClickHandledByServiceLogic) { // Se foi um delete tratado
            ...
            // ... (lógica de state.currentAction e pré-processamento mantida, mas adicionando actorClient.id nas chamadas de serviço quando apropriado)
            ...
            // ... (lógica de limpeza de currentAction mantida)
            ...
            let actionWasAnEdit = false; // Renomeei para evitar conflito de escopo
            ...
            if (aiResponse.detected_actions && aiResponse.detected_actions.length > 0) {
                for (const detectedAction of aiResponse.detected_actions) {
                    ...
                    // --- VERIFICAÇÕES DE PERMISSÃO E CONTEXTO ---
                    ...
                    // Adicionar mais verificações de sharedAccessPermissions aqui se necessário para cada ação.
                    // Ex: if (state.isSharedAccessContext && !state.sharedAccessPermissions.canCreateTransactions && actionName === 'CREATE_FINANCIAL_TRANSACTION') block...
                    
                    // ... (Restante das verificações de plano, conta ativa, tipo de conta PJ/MEI mantidas) ...
                    ...
                    try {
                        switch (actionName) {
                            // ... (Cases CREATE_FINANCIAL_TRANSACTION, UPDATE_FINANCIAL_TRANSACTION, SCHEDULE_APPOINTMENT, etc.
                            //      com actorId passado para os services e verificações de permissão se necessário)

                            // Implementação do SWITCH_FINANCIAL_ACCOUNT
                            case 'SWITCH_FINANCIAL_ACCOUNT': {
                                ...
                                if (accountsForSwitch.length === 0) {
                                    ...
                                    platformLinkFooter = "";
                                    if (isOwnerActingOnOwnBehalf) { state.data.onboardingStage = 'setting_up_pf_account_name'; state.currentAction = 'awaiting_input_pf_name';}
                                    break;
                                }
                                 if (accountsForSwitch.length === 1 && accountsForSwitch[0].id === state.activeFinancialAccountId) {
                                    ...
                                    currentActionFormattedData = "Não há outra para trocar no momento."; platformLinkFooter = "";
                                    break;
                                }
                                ...
                                if (foundAccount && foundAccount.id !== state.activeFinancialAccountId) {
                                    ...
                                    // A IA já deve ter fornecido uma boa overall_summary_suggestion para a troca.
                                ...
                                if (foundAccount) state.currentAction = null; // Limpa currentAction se a troca foi bem sucedida ou já estava na conta.
                                break;
                            }
                            // ... (Restante dos cases, incluindo os de BusinessClient, SharedAccess, etc., com traduções de status)
                            case 'CREATE_FINANCIAL_ACCOUNT': {
                                // ... (Mantido, mas lembrando que é para o DONO - actorClient.id deve ser o ownerId)
                                ...
                                const currentClientAccounts = await clientService.getClientFinancialAccounts(actorId, { isActive: true }); // Contas do ATOR (dono)
                                ...
                                if (aiResponse.detected_actions.length === 1 && (!aiMessageIntro || aiMessageIntro.startsWith("Ok,"))) aiMessageIntro = aiResponse.overall_summary_suggestion || `Conta "${newFinancialAccount.accountName}" (${newFinancialAccount.accountType}) criada e selecionada, ${clientNameToUse}!`;
                            ...
                            default:
                                // (Fallback mantido, mas o warning deve ser mais raro com mais cases)
                                ...
                                if (multipleActionBodiesList.length === 0 &&
                                    ...
                                    // Mantém
                                } else if (aiResponse.overall_summary_suggestion && (typeof aiMessageIntro !== 'string' || aiMessageIntro.startsWith(`Ok, ${clientNameToUse}!`))  && !aiResponse.overall_summary_suggestion.startsWith("Ok,")) {
                ...
                // ... (Lógica de montagem da mensagem e envio mantida, incluindo botões)
                 if (multipleActionBodiesList.length > 0) {
                    ...
                    if ((typeof aiMessageIntro === 'string' && (aiMessageIntro.startsWith(`Ok, ${clientNameToUse}!`) || !aiMessageIntro.includes(clientNameToUse))) &&
                        aiResponse.overall_summary_suggestion && !aiResponse.overall_summary_suggestion.startsWith(`Ok, ${clientNameToUse}!`) && // Evita substituir intro boa por "Ok"
            ...
            if (aiResponse.clarifications_needed && aiResponse.clarifications_needed.length > 0) {
                // Se a intro já foi definida por uma ação anterior (múltiplas ações) e agora pede clarificação, não sobrescreve a intro.
                if(!(multipleActionBodiesList.length > 0 && aiMessageIntro && !aiMessageIntro.startsWith("Ok,"))) {
                    aiMessageIntro = aiResponse.reply_to_user_suggestion || `Opa, ${clientNameToUse}! Para continuarmos, preciso de um detalhe:`;
                }
                // Adiciona a pergunta de clarificação ao corpo, mesmo que já haja algo de ações anteriores.
