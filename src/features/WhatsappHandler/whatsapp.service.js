    ...
    try {
        ...
        if (rawPayload && rawPayload.selectedButtonId && typeof rawPayload.selectedButtonId === 'string') {
            ...
            if (buttonId.startsWith('edit_transaction_')) {
                ...
                const cardId = parseInt(parts[parts.length -1], 10); 
                const cardNameFromState = state.data.cardNameForInvoice || "o cartão selecionado"; 
        ...
        if (aiResponse.detected_actions && aiResponse.detected_actions.length > 0) {
            for (const detectedAction of aiResponse.detected_actions) {
                ...
                let currentActionFormattedForLoop = ""; 
                    ...
                    switch (detectedAction.action) {
                        ...
                        case 'GET_CREDIT_CARD_INVOICE': {
                            ...
                            if (params.invoicePeriodType === 'especifico' && params.invoiceMonth && params.invoiceYear) {
                                ...
                                periodOpts.month = params.invoiceMonth;
                                periodOpts.year = params.invoiceYear;
                        ...
                        case 'PAY_CREDIT_CARD_INVOICE': {
                            ...
                            if (!cardIdForPayment) {
                                throw new Error(`Não identifiquei o cartão "${params.creditCardName}" para registrar o pagamento da fatura, ${clientNameToUse}. Verifique o nome, por favor. 🙏`);
                            }
        ...
        if (actionErrorOccurred) {
            ...
            if (finalReplyParts.length > 0 && messageForInvoiceSelectionDisplay && !finalReplyParts[0].includes(messageForInvoiceSelectionDisplay.substring(0,20))) {
                 finalReplyParts.push(messageForInvoiceSelectionDisplay); 
            } else if (messageForInvoiceSelectionDisplay) { 
                finalReplyParts = [messageForInvoiceSelectionDisplay]; 
            } else {
                finalReplyParts.push(`Por favor, escolha um período para a fatura do cartão ${state.data.cardNameForInvoice || 'selecionado'}, ${clientNameToUse}.`);
                 logger.warn("Estado 'awaiting_invoice_period_selection' mas messageForInvoiceSelectionDisplay não foi definida como esperado.");
            }
        }
            ...
            if (aiResponse.reply_to_user_suggestion && 
                ...
                finalReplyParts[finalReplyParts.length-1] !== aiResponse.reply_to_user_suggestion ) { 
            ...
            if (finalReplyParts.length > 0) { 
                if(aiResponse.reply_to_user_suggestion && aiResponse.reply_to_user_suggestion.length > 5 && (!aiResponse.overall_summary_suggestion || !aiResponse.reply_to_user_suggestion.includes(aiResponse.overall_summary_suggestion.substring(0,15)))){
                     finalReplyParts.push(aiResponse.reply_to_user_suggestion);
                }
        ...
        if (actionWasAnEdit || (state.editingResource && aiResponse.detected_actions?.every(a => !a.action || !a.action.startsWith("UPDATE_")))) { 
            state.editingResource = null;
        }
