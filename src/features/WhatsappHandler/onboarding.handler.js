...
async function handleOnboardingStep(state, messageText, actorClient) {
    let onboardingReply = "";
    const clientName = (actorClient.name && actorClient.name !== 'Convidado') ? actorClient.name.split(" ")[0] : (state.pushNameFromPayload || "você");
    ...
    if (state.data.onboardingStage === 'awaiting_plan_confirmation') {
        ...
        state.currentAction = 'awaiting_plan_interest_generic';
         ...
         if (isSharedContext) { 
            ...
            onboardingReply = getOnboardingAskForPFAccountNameMessage(clientName);
            ...
            if (pfAccountName.length >= 3 && pfAccountName.length <= 50) {
                try {
                    ...
                    if (planTier === 'avancado') {
                        ...
                        onboardingReply = getOnboardingConfirmPJAccountSetupMessage(clientName, pfAccountName, state.accessLevelTextForUser);
                        state.currentAction = 'awaiting_pj_mei_confirm';
                    } else {
                        state.data.onboardingStage = 'onboarding_complete';
                        const aiIntro = `Conta Pessoal "${pfAccountName}" criada com sucesso, ${clientName}! 🏦`;
                ...
                onboardingReply = `Esse nome parece um pouquinho curto ou um cadinho longo demais, ${clientName}. Para sua conta Pessoal, que tal um nome entre 3 e 50 letras? Assim fica perfeito! ✍️`;
            }
        }
        ...
        if (isSharedContext) {
            ...
            onboardingReply = getOnboardingConfirmPJAccountSetupMessage(clientName, actorPFAccount?.accountName || "Pessoal", state.accessLevelTextForUser);
            state.currentAction = 'awaiting_pj_mei_confirm';
        } else { 
            ...
            if (wantsPjMei) {
                if (pjMeiType) {
                    ...
                    onboardingReply = getOnboardingAskForCompanyNameMessage(clientName, pjMeiType);
                    state.data.onboardingStage = 'creating_pj_mei_account_name';
                    state.currentAction = 'awaiting_input_pj_mei_name';
                } else {
                    onboardingReply = getOnboardingAskForPJTypeMessage(clientName);
                    state.data.onboardingStage = 'awaiting_pj_mei_type';
                    state.currentAction = 'awaiting_input_pj_mei_type';
                }
            } else {
                const aiIntro = `Tranquilo, ${clientName}! Sem pressa. Se mais pra frente você quiser adicionar sua conta empresarial, é só me avisar! 😉`;
        ...
        if (isSharedContext) { state.data.onboardingStage = 'onboarding_complete'; state.currentAction = null; }
            ...
            if (typeInput === 'PJ' || typeInput === 'MEI') {
                ...
                onboardingReply = getOnboardingAskForCompanyNameMessage(clientName, typeInput);
                state.data.onboardingStage = 'creating_pj_mei_account_name';
                state.currentAction = 'awaiting_input_pj_mei_name';
            } else {
                onboardingReply = `Por favor, ${clientName}, me diga se é "PJ" ou "MEI" para sua conta empresarial. Assim a gente configura tudo certinho! 😊`;
            }
        }
        ...
        if (isSharedContext) { state.data.onboardingStage = 'onboarding_complete'; state.currentAction = null; }
            ...
            if (companyName.length >= 3 && companyName.length <= 50) {
                try {
                    ...
                    onboardingReply = getOnboardingCompanyCreatedMessage(clientName, companyType, companyName, personalAccountName);
                ...
                onboardingReply = `Para o nome da sua ${companyType}, ${clientName}, que tal algo entre 3 e 50 letras? Assim fica bem bacana! 🌟`;
            }
        }
    }
