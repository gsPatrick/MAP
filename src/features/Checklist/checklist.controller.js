// src/features/Checklist/checklist.controller.js (Novo Arquivo)
...
async function updateItem(req, res, next) {
  try {
    // financialAccountId vem do middleware de autorização
    const { financialAccountId } = req; 
    const { itemId } = req.params;
    const item = await checklistService.updateChecklistItem(financialAccountId, parseInt(itemId, 10), req.body);
...
async function deleteItem(req, res, next) {
  try {
    const { financialAccountId } = req;
    const { itemId } = req.params;
    await checklistService.deleteChecklistItem(financialAccountId, parseInt(itemId, 10));
    res.status(204).send();
  } catch (error) {
    next(error);
  }
